...
Peritus platform operators whose `user_id` is present in `public.super_admins`. The admin surface at `/settings` is gated by `is_super_admin(auth.uid())`, and the `stripe-settings` edge function rejects calls from any other role. Stripe account access requires either the **Owner** or **Administrator** role on the Mithras Peritus Stripe account.
...
- You hold the Stripe account credentials in Passbolt under `External Services → Stripe → Peritus account`. Never paste the live secret key into Slack, email, or a ticket.
...
- Subscription lifecycle events are mirrored to `public.organizations` columns `stripe_status`, `stripe_subscription_id`, `stripe_current_period_end`, and `subscription_expires_at`. These records are retained for the lifetime of the customer relationship plus seven years in accordance with the Peritus financial records retention policy.
