...
audience: partner
description: Read the AI Triage Agent verdict and the Commander autonomous response, then confirm, roll back, or escalate to the Peritus 24/7 SOC.
order: 6
...
## Purpose
This procedure governs the reseller response to an open incident raised by the AI Triage Agent and the Commander. The reseller confirms the autonomous action where the verdict is sound, rolls back where it is not, and escalates to the Peritus 24/7 SOC where the chain failed or the outcome is ambiguous. The procedure preserves customer protection, prevents unnecessary outage, and ensures every action is recorded for review.
...
- After `Escalate`, the incident header shows `Escalated to Peritus 24/7 SOC` with the assignment timestamp and the assigned operator.
...
- **The `Adversarial` agent returned `refuted` but the action ran.** A Peritus 24/7 SOC operator forced the action. The `forceFire override` badge is present on the `Autonomous response` card. Contact the named operator before reversing.
