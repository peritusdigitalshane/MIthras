...
- Their credit balance in `/distributor/credits` matches what you cut.
- After they sign in, your reseller-health view shows them as **Connected** (last_login_at populated).

## Troubleshooting
- **Activation email didn't arrive.** Check `/activity` for a row with `action='invite_user'`. If the row is there but the email never landed, the SMTP send failed — re-issue from the row, or have the reseller use **Forgot password** at the login page to set their own password.
- **Credit cut shows "insufficient_balance".** Your own distributor pool is depleted. Open **`/distributor/credits`** → contact-Peritus tile and request a top-up before continuing.
