# ApiClient.psm1 — HTTP client for the Mithras Threat Defence Agent API.
...
function Invoke-AgentHeartbeat {
    ...
    # See note in Invoke-AgentAppControlObserved -- pass body as UTF-8 bytes so
    # signed canonical JSON == bytes on the wire (works even with non-ASCII).
...
function Invoke-AgentAppControlObserved {
    ...
    # PowerShell 5.1's Invoke-RestMethod -Body <string> encodes via the system
    # codepage when ContentType doesn't carry an explicit charset. Body bytes
    # then differ from what we signed (UTF-8), breaking HMAC for any payload
    # containing non-ASCII (e.g. "Microsoft (R)" in publisher fields). Pass the
    # body as UTF-8 bytes explicitly to guarantee exact byte equality with what
    # was signed.
    $bodyBytes = [System.Text.Encoding]::UTF8.GetBytes($rawBody)
    return Invoke-RestMethod -Uri $url -Method Post `
        -ContentType 'application/json; charset=utf-8' `
