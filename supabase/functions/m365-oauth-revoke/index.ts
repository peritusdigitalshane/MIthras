// POST /functions/v1/m365-oauth-revoke
//
// Operator-triggered revoke of a single OAuth grant. Requires:
//   - user JWT
//   - caller is org admin or super-admin of the target org
//   - tenant has remediation_enabled=true (Application.ReadWrite.All)
//
// Calls DELETE /oauth2PermissionGrants/{id} on Graph; on success, marks the
// m365_oauth_grants row revoked. 404 from Graph is treated as success
// (grant already gone).
//
...
Deno.serve(async (req: Request) => {
    ...
    let body: { grant_id?: string; reason?: string };
    try { body = await req.json(); } catch { return json({ error: "invalid_body" }, 400, origin); }
    if (!body.grant_id) return json({ error: "grant_id_required" }, 400, origin);

    const { data: row } = await supabase
        .from("m365_oauth_grants")
        .select("id, grant_id, organization_id, m365_tenant_id, client_display_name, revoked_at, deleted_at")
        .eq("id", body.grant_id)
        .maybeSingle();
    if (!row) return json({ error: "grant_not_found" }, 404, origin);
    if (row.revoked_at) return json({ error: "already_revoked" }, 409, origin);
    if (row.deleted_at) return json({ error: "already_deleted" }, 409, origin);
    ...
    if (!t.remediation_enabled) {
        return json({
            error: "remediation_not_enabled",
            detail: "M365 tenant must have remediation consent for Mithras to revoke OAuth grants.",
        }, 409, origin);
    }
    ...
    const resp = await fetch(`https://graph.microsoft.com/v1.0/oauth2PermissionGrants/${row.grant_id}`, {
        method: "DELETE",
    ...
    await supabase.from("m365_oauth_grants").update({
    ...
    await supabase.from("activity_logs").insert({
        ...
        action: "m365_oauth_grant_revoked",
        resource_type: "m365_oauth_grant",
